...
__global__ void matrix_multiply_kernel(unsigned char *temp, unsigned char *matrix, float *kernal, int order, int windowSizeX, int windowSizeY){
    int middle = ceil(order/2);
    int threadID = blockIdx.x * blockDim.x + threadIdx.x;
    int x, y;

    if(y >= 0 && y < windowSizeY){
...
void multiplyKernal(unsigned char* matrix, float* kernal, int order, int windowSizeX, int windowSizeY){
    ...
    matrix_multiply_kernel<<<grid_dim, block_dim>>>(temp, matrix_d, kernal_d, order, windowSizeX, windowSizeY);
